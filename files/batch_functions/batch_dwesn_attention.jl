...
function do_batch_dwesn_attention(_params_esn, _params)

    p,pe          = _params, _params_esn
    l_r           = vcat(p[:layers]...)
    szl           = length(l_r)
    fsz           = (id) -> sum( [ l_r[i] for i in 1:szl if id in keys(p[:connections]) && i in getfield.(filter(x -> x[2] != 0, p[:connections][id]), 1) ] ) + get_input_size(id,p)
    id_counter    = 0
    layers        = []
